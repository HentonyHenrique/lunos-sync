...
begin
  ArqIni := TIniFile.Create('config.ini');
  if frmAuth.cnpj_logado = ArqIni.ReadString('EMPRESA', 'CNPJ', '') then
  begin
    ...
    begin
      lbErroCNPJ.Caption := 'Usuário não possui acesso a esse CNPJ';
    end;
  end
  else
  begin
    showMessage
      ('CNPJ do usuário logado difere do CNPJ informado na configuração, o sistema será encerrado');
    exitprocess(0);
  end;
